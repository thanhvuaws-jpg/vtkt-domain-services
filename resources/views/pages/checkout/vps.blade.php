                                ...
                                <div class="py-5">
                                    <div class="form fv-plugins-bootstrap5 fv-plugins-framework">
                                        <div class="fv-row mb-10">
                                            <label class="required fw-semibold fs-6 mb-2"> Gói VPS </label>
                                            <input type="text" class="form-control form-control-solid" value="{{ $vps->name }}" disabled>
                                        </div>
                                        <div class="fv-row mb-10">
                                            <label class="fw-semibold fs-6 mb-2"> Cấu Hình </label>
                                            <textarea class="form-control form-control-solid" style="height: 100px" disabled>{{ $vps->description ?? '' }}</textarea>
                                        </div>
                                        <div class="fv-row mb-10">
                                        ...
                                        <button id="buy" type="button" class="btn btn-primary w-100 py-4">
                                            <span class="indicator-label fs-3 fw-bold">Mua Ngay - <span id="price-display">{{ number_format($vps->price_month) }}</span>đ</span>
                                        </button>
                                    </div>
                                </div>
                            </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="modal-title" id="modalTitle">Đang thiết lập VPS của bạn...</h3>
            </div>
            <div class="modal-body text-center py-10">
                ...
                <div id="successArea" style="display: none;">
                    ...
                    <h4 class="text-success fw-bold mb-5">Đã cấp phát VPS thành công!</h4>
                    <div class="bg-light p-5 rounded text-start mb-5" style="border: 1px dashed #ccc;">
                        <div class="mb-3">
                            <span class="text-muted">Địa chỉ IP:</span>
                            <span class="fw-bold ms-2 text-primary" id="vpsIp">---</span>
                        </div>
                        <div class="mb-3">
                            <span class="text-muted">Tài khoản:</span>
                            <span class="fw-bold ms-2" id="vpsUser">---</span>
                        </div>
                        <div>
                            <span class="text-muted">Mật khẩu:</span>
                            <span class="fw-bold ms-2 user-select-all" id="vpsPass" style="background:#fff; padding:2px 5px; border-radius:4px;">---</span>
                        </div>
                    </div>
                </div>
            </div>
...
<input type="hidden" id="price-month" value="{{ $vps->price_month }}">
<input type="hidden" id="price-year" value="{{ $vps->price_year }}">
                    ...
                    if (data.credentials) {
                        ...
                        var interval = setInterval(function() {
                            ...
                            if (progress >= 100) {
                                ...
                                setTimeout(function() {
                                    $('#progressArea').hide();
                                    $('#modalTitle').text('Hoàn Tất Khởi Tạo!');
                                    $('#successArea').fadeIn();
                                    $('#modalFooter').fadeIn();
                                    $('#vpsIp').text(data.credentials.ip);
                                    $('#vpsUser').text(data.credentials.username);
                                    $('#vpsPass').text(data.credentials.password);
                                }, 500);
                            }
                        }, 400);
